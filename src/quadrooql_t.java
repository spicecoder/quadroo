...
//users,words and quadroo
	quadrooql_t(String url) throws ClassNotFoundException {
...
if (wi > 0) {
	...
	//select sid from quadroo where  fid=" +wi + " AND sid > 0 ; 
	 rsf 	= statement.executeQuery("select sid  from quadroo where  fid==" + wi );
	//System.out.println("rsf count:"+ rsf.getFetchSize()+ "f:"+ rsf.next());
	if (rsf.next() )  { String xml = "<flow name = " + W + ">";
	                     wi = rsf.getInt("sid");
	                     System.out.println(" for sid:"+ wi);
	                   //  wrd = GWord(wi);
	                     xml = xml + "<scene name=" + wi+ "/>";  
	 // System.out.println("rsf:"+ rsf.getFetchSize() + "v"+ rsf.next());  
		while (rsf.next()  ){
...
public static void main(String[] args) throws ClassNotFoundException { 
	String url = "jdbc:sqlite:qudtestto.db";
	quadrooql_t ql = new quadrooql_t(url);
	//ql.AWord(1, "firstword");
	ql.AWord(1, "morning");
	ql.AWord(1, "goodmorning");
	System.out.println("getWord:"+ql.GWord(6));
	System.out.println("getWord0:"+ql.GWord(0));
	System.out.println("listtScenes:"+ql.LFScenes("virginword1"));
